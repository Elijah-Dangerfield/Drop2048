...
android {
    namespace = "com.dangerfield.drop2048.features.stats.impl"
...
// Compare by default, exactly as the other three harnesses do, and for the reason
// L39 records: `captureRoboImage` is a no-op unless a flag is set, so a plain
// `testDebugUnitTest` runs every screenshot test, passes every one of them, and
// never looks at a pixel. Verified here the way C2c verified it there — by
// swapping a golden for a different image and watching the task fail, naming the
// test.
