 ...
 * it is SPEC 10's rule made specific: a force-update gate is the only config
 * value that can brick every install at once, and the binary must be fully
 * playable with the server unreachable. Every key behind this defaults to 0 or
 * `off`.
 *
 * The inputs are plain numbers and strings rather than `ConfiguredValue`s, so
 * the whole decision is testable without a config map, a DI graph or a device.
 * `LaunchGatesTest` is mostly a list of ways an outage must *not* be able to
 * reach a wall.
 */

/** The three words `upgrade.maintenanceMode` may hold. Anything else is [MaintenanceOff]. */
const val MaintenanceOff = "off"
const val MaintenanceBanner = "banner"
const val MaintenanceBlocking = "blocking"
 ...
 * unsupported build that we are doing maintenance sends them back tomorrow to
 * the same wall. Legal is last because consent to keep using an app is worth
 * nothing while the app is unusable anyway.
 *
...
fun resolveLaunchGates(
    ...
    val maintenanceBlock = maintenance.messageFor(MaintenanceBlocking)
    ...
    val maintenanceNotice = maintenance.messageFor(MaintenanceBanner)
 ...
 *
 * Casing is forgiving because the console takes raw text and `Blocking` is not a
 * mistake worth punishing. Anything that is not one of the three declared words
 * resolves to off.
 */
