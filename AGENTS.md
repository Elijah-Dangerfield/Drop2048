...
Guidelines for AI agents working in the Drop 2048 repository.
...
| Plugin | Use |
|--------|-----|
| `drop2048.kotlin.multiplatform` | Pure Kotlin |
| `drop2048.compose.multiplatform` | Kotlin + Compose |
| `drop2048.feature` | Feature modules |
| `drop2048.application` | apps:compose only |
...
For singletons where the warm path matters (app-lifecycle dispatchers, disk-backed repositories, anything whose `init {}` is load-bearing), implement [`AutoInit`](libraries/core/src/commonMain/kotlin/com/drop2048/libraries/core/AutoInit.kt) and contribute a second binding via multibinding:
