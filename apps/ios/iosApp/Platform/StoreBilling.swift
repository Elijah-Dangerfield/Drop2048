...
//  iosApp
//
//  StoreKit 2, ported from Sodogku. No SDK to add — StoreKit ships with the OS
//  — so unlike `AdNetwork.swift` this is unguarded and real from the first
//  build.
//
//  Until this existed, iOS fell through to `NoStoreBilling`, whose `purchase()`
//  answers `Unavailable` / `no_store_wired`. Nothing in `features/paywall` is
//  platform-gated, so the Pro sheet rendered on iOS with a button that could
//  not take money. That is App Review guideline 2.1, and it was the single
//  largest thing between this app and a submission.
//
...
//  answer clears a paying customer's entitlement. `RealEntitlements` treats
//  `.unknown` as "keep whatever was cached".
//
//  ── SETUP FOR LOCAL TESTING ──────────────────────────────────────────────
//  A product id App Store Connect has never heard of returns an empty product
//  list, and the paywall then shows no price. For the simulator:
//
//    File ▸ New ▸ File ▸ StoreKit Configuration File (Products.storekit)
//    Add a Non-Consumable with product id `drop2048_pro` (ProductIds.pro),
//    price 2.99, then Product ▸ Scheme ▸ Edit Scheme ▸ Run ▸ Options ▸
//    StoreKit Configuration ▸ Products.storekit.
//
//  For a sandbox test on a device, the non-consumable in App Store Connect is
//  enough; sign in with a Sandbox Apple Account.
//
import ComposeApp
import Foundation
import StoreKit
