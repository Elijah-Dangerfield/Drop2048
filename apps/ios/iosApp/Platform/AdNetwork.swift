...
//  unit ids in `AdUnits.kt`. The app id is the real one unconditionally,
//  because a plist value is fixed at build time and cannot branch on a runtime
//  channel; the units follow `AdUnits.useTestUnits`, which answers from
//  `BuildInfo.releaseChannel`, so only a `store` build requests live inventory.
//
...
class IOSAdNetwork: NSObject, AdNetwork {
    ...
    /// The controller an ad or a consent form should be presented from: the
    /// **top of the presentation chain**, not the window root.
    ///
    ...
    ///
    /// Ported from Sodogku, which found both after Sentry caught a Compose host
    /// stuck below STARTED with `GADFullScreenAdViewController` on screen and
    /// navigation commands queued behind it. That does not prove presentation
    /// was the cause there, and the watchdog half of its fix is not ported:
    /// `docs/todos.md` says what to do if this app ever gets a "it froze after
    /// an ad" report.
    @MainActor
