...
deploy to [Fly.io](https://fly.io). Postgres lives on Supabase, not Fly.
...
```bash
# From the repo root:
docker build -f apps/server/Dockerfile -t drop2048-server .
docker run --rm -p 8080:8080 -e DATABASE_URL=... drop2048-server
```
...
is the one that unlocks remote config — the server boots without it (limited
mode) so a misconfigured deploy still answers `/_health`.
