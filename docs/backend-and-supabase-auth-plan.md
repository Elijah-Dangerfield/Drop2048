...
Package root: **`com.dangerfield.drop2048.server`** (keep dir == package; do **not**
...
apps/server/
  ...
  src/main/kotlin/com/drop2048/server/
  ...
  src/main/resources/
    ...
    logback.xml
  src/test/kotlin/com/drop2048/server/
...
- **build-logic**: make the KMP convention plugins skip Android/iOS targets when
  `-Ddrop2048.serverOnly=true` (mirrors Cards) so a server Docker build needs
  no mobile toolchain.
