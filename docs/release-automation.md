...
- **`versionCode` / iOS build number** — auto-overridden in CI with `GITHUB_RUN_NUMBER` via env vars `VERSION_CODE_OVERRIDE` and `BUILD_NUMBER_OVERRIDE`. They bump monotonically without commits. (See [Versioning.kt](../build-logic/src/main/java/com/drop2048/util/Versioning.kt).)
- **Sentry release ID**: `drop2048@{version}+{build}` (e.g. `drop2048@0.2.0+42`). Created on both platforms in release.yml.
...
| Name | Kind | Value |
|---|---|---|
| `ANDROID_KEYSTORE_BASE64` | secret | `base64 -i drop2048-upload.p12 \| pbcopy` |
| `ANDROID_KEYSTORE_PASSWORD` | secret | Set when keystore was created. |
| `ANDROID_KEY_ALIAS` | secret | e.g. `drop2048-release`. |
...
```sh
keytool -genkey -v -keystore drop2048-upload.p12 -storetype PKCS12 \
  -keyalg RSA -keysize 4096 -validity 10000 -alias drop2048-release
```
...
3. IAM & Admin → Service Accounts → Create → name `drop2048-ci`, role none.
...
- [ ] Create the app entry for `com.dangerfield.drop2048`
