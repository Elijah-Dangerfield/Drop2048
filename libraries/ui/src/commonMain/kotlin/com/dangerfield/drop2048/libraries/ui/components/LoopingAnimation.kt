 ...
 *    capture that waits for idle spins producing nothing. It does not time out
 *    and it does not fail — it just never finishes, which reads as a hung build
 *    rather than as a bad test, and is far harder to trace back to the one
 *    animating component that caused it.
 ...
 *    a false positive.
 *
 * Returning `State` rather than a `Float` is the other half of the contract:
 * read `.value` inside a `graphicsLayer`, `drawBehind` or `Canvas` lambda so the
 * animation only invalidates *draw*. Unwrapping it with `by` at the call site
 * subscribes composition to a value that changes every frame, which is what
 * `AnimatedStateReadInComposition` fails the build over.
 *
