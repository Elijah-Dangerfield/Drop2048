 ...
 * written in Kotlin against the GameKit platform bindings rather than as a Swift
 * object handed through the iOS component. GameKit is an ordinary Objective-C
 * system framework, so Kotlin/Native already has full bindings for it and a
 * Swift shim would only add a file to keep in sync.
 *
 * **Android binding** is `NoGameServices`, which is inert. Play Games is the
 * equivalent and is not in v1.
 *
