 ...
 * parked off the edge of the screen, and the switch that would hide it again is
 * behind a Settings row the tester has to know about. Getting the arithmetic
 * wrong is recoverable only by finding that row or reinstalling.
 */
class DevFeedbackFabPlacementTest {
    ...
    fun aFreshInstallStartsSomewhereItCanBeTapped() {
        ...
        assertTrue(!fresh.hidden, "the button is the way in, so it starts shown")
    }
