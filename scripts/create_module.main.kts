...
fun main() {
    printBlue("🚀 Drop 2048 Module Creator")
    println()
...
fun checkForHelp(): Boolean {
    ...
    if (isHelpRequest) {
        printBlue("""
            Drop 2048 Module Creator
            
            This script creates new KMP modules for the Drop 2048 project with proper structure and configuration.
...
fun createSourceStructure(config: ModuleConfig, moduleDir: File) {
    val packagePath = "com.dangerfield.drop2048.${config.baseDir}.${config.fullName.replace(":", ".")}".replace(".", "/")
...
fun createMainSourceFile(config: ModuleConfig, kotlinDir: File) {
    val packageName = "com.dangerfield.drop2048.${config.baseDir}.${config.fullName.replace(":", ".")}"
...
fun createFeatureEntryPoint(config: ModuleConfig, kotlinDir: File, packageName: String) {
    ...
    val (routePackage, routeClass) = if (config.moduleName == "impl" && config.parentModule != null) {
        ...
        val parentPackage = "com.dangerfield.drop2048.${config.baseDir}.${config.parentModule}"
...
${routeImport}import com.dangerfield.drop2048.libraries.navigation.FeatureEntryPoint
...
fun createBuildGradle(config: ModuleConfig, moduleDir: File) {
    ...
    val pluginId = when (config.type) {
        ModuleType.FEATURE -> "drop2048.feature"
        ModuleType.LIBRARY -> "drop2048.kotlin.multiplatform"
    }
    
    val namespace = "com.dangerfield.drop2048.${config.baseDir}.${config.fullName.replace(":", ".")}"
